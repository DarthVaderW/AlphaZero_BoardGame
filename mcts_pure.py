...
def rollout_policy_fn(board) -> Iterable[Tuple[int, float]]:
    ...
    Returns a zip of (action, probability) pairs with random probabilities.
    """
    # rollout randomly
...
def policy_value_fn(board) -> Tuple[Iterable[Tuple[int, float]], float]:
    """A function that takes in a state and outputs a list of
    (action, probability) tuples and a score for the state.

    For pure MCTS, returns uniform probabilities and 0 score.
    """
...
class TreeNode(object):
    ...
    def expand(self, action_priors: Iterable[Tuple[int, float]]) -> None:
        ...
        action_priors: iterable of (action, prior probability) from policy.
        """
...
class MCTS(object):
    ...
    def __init__(self, policy_value_fn: Callable, c_puct: float = 5, n_playout: int = 10000):
        """
        policy_value_fn: a function that takes in a board state and outputs
            a list of (action, probability) tuples and also a score in [-1, 1]
            for the current player.
        c_puct: controls trade-off between prior and value.
        n_playout: number of playouts per move.
        """
    ...
    def _evaluate_rollout(self, state, limit: int = 1000) -> int:
        ...
        Returns +1 if current player wins, -1 if the opponent wins,
...
class MCTSPlayer(object):
    ...
    def __init__(self, c_puct: float = 5, n_playout: int = 2000):
